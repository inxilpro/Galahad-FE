...
class Galahad_Service_Ooyala extends Zend_Service_Abstract
{
	...
	public function deleteMetadata($embedCode, array $keys)
	{
		...
		return $this->_request('set_metadata', array('embedCode' => $embedCode, 'delete' => $keys));
	}
	...
	public function getSubLabels($label)
	{
		...
		return $this->_request('labels', array('mode' => 'listSubLabels', 'label' => $label));
	}
	 ...
	 * @return Galahad_Service_Ooyala_Response
	 */
	public function getChannelItems($embedCode)
	{
		return $this->_request('channels', array('mode' => 'list', 'channelEmbedCode' => $embedCode));
	}
	...
	protected function _request($method, array $params = array(), $responseType = null)
	{
		...
		if (null !== $responseType) {
			$responseClass .= "_{$responseType}";
			
			/** @see Zend_Loader */
			require_once 'Zend/Loader.php';
			Zend_Loader::loadClass($responseClass);
			if (!class_exists($responseClass)) {
				$this->_throwException("Response type '{$responseType}' does not exist!");
			}
		}
		
		$response = new $responseClass($client->request());
		$response->setRequest($params); // TODO: Maybe pass client too?
